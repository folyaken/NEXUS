...
// Пока правил нет, вкладка объясняет, что происходит: пустой экран без
// пояснений читается как поломка.
assert.match(jey, /routing-empty-state/);
assert.match(jey, /\{t\('Правил пока нет'\)\}/);
for (const phrase of ['Справочник имён', 'Маршрутизация', 'Правил пока нет']) {
  assert.equal(hasTranslation('en', phrase), true, `нужен перевод: «${phrase}»`);
}
