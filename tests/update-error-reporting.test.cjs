...
assert.match(limits, /maxRequests: 24,/, 'бюджет должен учитывать повторы по User-Agent для обеих целей');

// Текст ошибки обязан объяснять, что делать, а не просто фиксировать формат.
assert.match(vpnManager, /Панель отдала только страницу входа/, 'ошибка должна называть причину');
assert.match(vpnManager, /Копировать подписку/, 'ошибка должна подсказывать конкретное действие');
