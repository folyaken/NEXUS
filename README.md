...
В **Jey2Ray → Настройки → Общие** есть переключатель «Раздача в локальную сеть». Когда он включён, локальные SOCKS/HTTP-входы поднимаются на `0.0.0.0`, и любое устройство домашней сети (ТВ, консоль, телефон) может ходить через NEXUS, указав IP компьютера и порт. Готовые адреса `ip:порт` показываются в карточке и в диагностике, дублируются пунктом в трее.

По умолчанию раздача выключена: прокси слушает только `127.0.0.1`. Включайте её лишь в доверенной сети — вход открыт без пароля; Windows может дополнительно запросить разрешение в брандмауэре.
...
Прогон всех регрессионных тестов:
...
NEXUS — проприетарное программное обеспечение. Copyright © 2026 NEXUS.
Все права защищены. Условия использования приведены в файле [LICENSE](LICENSE).

Сетевые ядра и модули (Xray-core, sing-box, Zapret, TG WS Proxy) — самостоятельные
программы сторонних разработчиков. Они не входят в состав установщика: NEXUS
загружает их с официальных репозиториев и запускает отдельными процессами.
Перечень компонентов, их правообладателей и лицензий приведён в файле
