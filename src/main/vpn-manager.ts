...
export class VpnManager extends EventEmitter {
  ...
  private async importSubscriptionUnlocked(url: string): Promise<VpnProfile[]> {
    ...
    if (!material.links.length && !material.clash.length) {
      throw new Error('Панель отдала только страницу входа. Откройте ссылку в браузере, нажмите «Получить ссылку» либо «Копировать подписку» и вставьте сюда скопированный адрес. Если на странице есть выбор приложения, подойдёт ссылка для любого клиента на Xray или Clash.');
    }
