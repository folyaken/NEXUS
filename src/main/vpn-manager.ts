...
export class VpnManager extends EventEmitter {
  ...
  private async importSubscriptionUnlocked(url: string): Promise<VpnProfile[]> {
    ...
    if (!material.links.length && !material.clash.length) {
      throw new Error('Панель вернула веб-страницу вместо конфигурации. Скопируйте ссылку кнопкой «Копировать подписку» в личном кабинете — ссылка на страницу входа не подходит. Если подписка требует авторизации, откройте её в браузере и скопируйте адрес уже готовой конфигурации.');
    }
